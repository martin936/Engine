...
void CTAA::Init()
{
	...
	ms_pHistoryIn	= new CTexture(nWidth, nHeight, e_R11G11B10_FLOAT);
	ms_pHistoryOut 	= new CTexture(nWidth, nHeight, e_R11G11B10_FLOAT, eTextureStorage2D);

	if (CRenderPass::BeginCompute(ERenderPassId::e_TAA, "TAA"))
	{
		// TAA Resolve
		if (CRenderPass::BeginComputeSubPass("TAA Resolve"))
		{
		...
		// Copy
		if (CRenderPass::BeginGraphicsSubPass("TAA Copy"))
		{
