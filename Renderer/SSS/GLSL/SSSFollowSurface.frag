...
void main(void)
{
	...
	if (radius > 1.f && Index >= 0)
	{
		...
		for (int i = -4; i < 0; i++)
		{
			...
			float factor = smoothstep(1.f, 0.f, fStrength * (length(pos.xyz - PosOffset.xyz) - RadiusWS));
			...
			vec3 w = weight.rgb * (1.f - clamp(abs(testSample.a - Color.a) * 1e8f, 0.f, 1.f)) * factor;
